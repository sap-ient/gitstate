...
function FieldRow({ label, value, hint, action }) {
  return (
    <div className="flex items-center gap-4 py-3 border-b border-[var(--border)] last:border-0">
      ...
      <div className="text-sm font-mono text-[var(--text-faint)] truncate max-w-[200px]">{value ?? '—'}</div>
      {action ?? (
        <button className="text-xs text-[var(--brand-teal)] hover:text-[#5eead4] transition-colors duration-150 shrink-0">
          Edit
        </button>
      )}
    </div>
...
export default function Settings() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {/* Account section */}
      <SectionCard title="Account" description="Your personal account details.">
        {/* Avatar + identity row */}
        <div className="flex items-center gap-4 pb-4 mb-2 border-b border-[var(--border)]">
          ...
          <div className="flex-1 min-w-0">
            ...
            {user?.role && (
              <Badge color="teal" className="mt-1.5">{user.role}</Badge>
            )}
          </div>
          <Button
            variant="outline"
      ...
      >
        ...
        <FieldRow
          ...
          action={
            <Link to="/settings/billing" className="text-xs text-[var(--brand-teal)] hover:text-[#5eead4] transition-colors duration-150 shrink-0">
              Billing
            </Link>
          }
        />
      ...
      <SectionCard title="Integrations" description="Connected git platforms. (Wave C1)">
        <div className="flex items-center gap-3 py-3">
          <div className="w-8 h-8 rounded-[var(--radius-badge)] bg-[var(--bg)] border border-[var(--border)] flex items-center justify-center">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-[var(--text-faint)]">Not connected</p>
          </div>
